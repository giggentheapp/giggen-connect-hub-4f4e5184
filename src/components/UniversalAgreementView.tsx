...
import { Printer, ArrowLeft, Clock, MapPin, Banknote, Edit2, Save, X, PenTool, Calendar, Users } from 'lucide-react';
...
export default function UniversalAgreementView() {
      ...
      const { error } = await supabase
        ...
        .update({
          ...
          receiver_contact_info: {
            ...
            name: editedContacts.receiver.name,
            email: editedContacts.receiver.email,
            phone: editedContacts.receiver.phone
          }
        })
        .eq('id', bookingId);
  ...
  // Render standard booking content
  const renderStandardBookingContent = () => {
      ...
      <div className="space-y-2 print:space-y-1">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2 print:print-grid">
          {booking.event_date && (
            <div className="border-b pb-2 print:pb-1 print-compact">
              <h2 className="text-sm font-semibold mb-1 flex items-center gap-1">
                <Calendar className="h-3 w-3" /> Dato og tid
              </h2>
          ...
          {booking.venue && (
            <div className="border-b pb-2 print:pb-1 print-compact">
              <h2 className="text-sm font-semibold mb-1 flex items-center gap-1">
                <MapPin className="h-3 w-3" /> Spillested
              </h2>
              <div className="text-sm text-muted-foreground">
                {booking.venue}
          ...
          {(booking.audience_estimate || booking.ticket_price) && (
            <div className="border-b pb-2 print:pb-1 print-compact">
              <h2 className="text-sm font-semibold mb-1 flex items-center gap-1">
                <Users className="h-3 w-3" /> Publikum og prising
              </h2>
              <div className="text-sm text-muted-foreground space-y-0.5">
                {booking.audience_estimate && (
                  <p>Forventet publikum: {booking.audience_estimate} personer</p>
                )}
                {booking.ticket_price && (
                  <p>Billettpris: {booking.ticket_price} kr</p>
                )}
                {hasRevenueData && (
                  <p className="font-medium text-foreground">
                    Total brutto inntekt: {totalRevenue.toLocaleString('no-NO')} kr
                  </p>
                )}
              </div>
            </div>
        ...
        <div className="border-b pb-2 print:pb-1 print-compact">
          <h2 className="text-sm font-semibold mb-1 flex items-center gap-1">
            <Banknote className="h-3 w-3" /> Artist honorar
          </h2>
            ...
            {booking.door_deal ? (
              <div>
                ...
                <p className="text-muted-foreground">
                  Prosentandel: {booking.door_percentage || 'X'}% av total dørinntekt
                </p>
                {hasRevenueData && (
                  <p className="font-medium text-green-700 dark:text-green-400">
                    Artist får: {artistEarnings.toLocaleString('no-NO')} kr
                  </p>
                )}
              </div>
              ...
              <div>
                ...
                <p className="text-muted-foreground text-xs">
                  Partene har valgt å avtale økonomi utenfor GIGGEN.
                </p>
              </div>
            ) : (
              <div>
                ...
                <p className="text-muted-foreground">
                  Beløp: {booking.artist_fee || 'Ikke spesifisert'} kr
                </p>
              </div>
            )}
        ...
        {/* Portfolio */}
        {bookingId && user && (
          <div className="border-b pb-2 print:pb-1 print-compact no-print">
            <h2 className="text-sm font-semibold mb-1">Portefølje</h2>
  ...
  // Render teaching booking content
  const renderTeachingBookingContent = () => {
    return (
      <div className="space-y-2 print:space-y-1">
        {/* Basic Info */}
        <div className="border-b pb-2 print:pb-1 print-compact">
  ...
  const agreementTitle = isTeaching ? 'Undervisningsavtale' : 'Bookingavtale';
  const senderLabel = isTeaching ? 'Lærer' : 'Avsender';
  const receiverLabel = isTeaching ? 'Elev' : 'Mottaker';
    ...
    <div className="max-w-3xl mx-auto p-4 print:p-2 print:max-w-full">
      ...
      <div className="mb-4 no-print flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold">{agreementTitle}</h1>
      ...
      <div className="hidden print:block mb-2 border-b pb-2">
        <h1 className="text-lg font-bold">{agreementTitle}</h1>
        <p className="text-xs text-muted-foreground">
          Generert: {format(new Date(), 'dd.MM.yyyy')} | {booking.title}
        </p>
      </div>

      {/* Agreement Details - Render based on type */}
      {isTeaching ? renderTeachingBookingContent() : renderStandardBookingContent()}
      ...
      {(booking.venue || booking.address) && !isTeaching && (
        ...
        {isEditing ? (
          <div className="space-y-4 no-print">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                <h3 className="text-sm font-medium">{senderLabel}</h3>
                <Input
                  placeholder="Navn"
              ...
              {/* Receiver Editable */}
              <div className="space-y-2">
                <h3 className="text-sm font-medium">{receiverLabel}</h3>
                <Input
                  placeholder="Navn"
          ...
          <div className="grid grid-cols-2 gap-4 print:gap-8">
            ...
            <div className="text-sm">
              <h3 className="font-medium mb-1">{senderLabel}</h3>
            ...
            {/* Receiver Display */}
            <div className="text-sm">
              <h3 className="font-medium mb-1">{receiverLabel}</h3>
              <div className="space-y-0.5 text-xs">
                <div>
                  {receiverProfile ? (
                    <Link 
