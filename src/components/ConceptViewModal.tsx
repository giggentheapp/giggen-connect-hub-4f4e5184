  ...
  const loadConceptData = async (conceptId: string) => {
    ...
    try {
      ...
      const {
        data: filesData,
        error: filesError
      } = await supabase.from('concept_files').select('id, filename, file_type, file_url, title, created_at, mime_type').eq('concept_id', conceptId);
      if (filesError) {
      ...
      if (conceptData.tech_spec_reference) {
        const {
          data: techSpecData,
          error: techSpecError
        } = await supabase.from('profile_tech_specs').select('id, filename, file_url, file_type').eq('id', conceptData.tech_spec_reference).maybeSingle();
        if (techSpecError) {
      ...
      if (viewMode === 'owner' && conceptData.hospitality_rider_reference) {
        ...
        } = await supabase.from('hospitality_riders').select('id, filename, file_url, file_type').eq('id', conceptData.hospitality_rider_reference).maybeSingle();
        ...
        <DialogHeader>
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              {conceptIds.length > 1 && <div className="flex gap-1">
                  <Button variant="outline" size="sm" onClick={handlePreviousConcept} disabled={currentConceptIndex === 0}>
                    <ChevronLeft className="h-4 w-4" />
                  </Button>
                  <Button variant="outline" size="sm" onClick={handleNextConcept} disabled={currentConceptIndex === conceptIds.length - 1}>
              ...
              <CardContent className="space-y-4">
                ...
                {(availableDates.length > 0 || isIndefinite) && <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {isIndefinite ? <Badge variant="outline">
                          Ubestemt / Ved avtale
                        </Badge> : Array.isArray(availableDates) ? availableDates.filter(date => date).map((date: string, index: number) => <Badge key={date || `date-${index}`} variant="outline">
            ...
            {conceptFiles.length > 0 && <Card>
                ...
                <CardContent>
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {Array.isArray(conceptFiles) ? conceptFiles.filter(file => file && file.id).map(file => <div key={file.id} className="bg-muted/30 rounded-lg overflow-hidden">
                        
                        {/* Video Display */}
                        {(file.file_type === 'video' || file.mime_type?.startsWith('video/') || file.filename.match(/\.(mp4|webm|mov|avi)$/i)) && <div className="aspect-video bg-black">
                            <video controls className="w-full h-full" preload="metadata" crossOrigin="anonymous">
                        ...
                        {/* Image Display */}
                        {(file.file_type === 'image' || file.mime_type?.startsWith('image/') || file.filename.match(/\.(jpg|jpeg|png|gif|webp|svg)$/i)) && <div className="aspect-video bg-muted flex items-center justify-center overflow-hidden">
                             <img src={file.file_url} alt={file.title || file.filename} className="w-full h-full object-cover cursor-pointer hover:scale-105 transition-transform" crossOrigin="anonymous" onClick={() => window.open(file.file_url, '_blank')} />
                           </div>}
                        
                        {/* Audio Display */}
                        {(file.file_type === 'audio' || file.mime_type?.startsWith('audio/') || file.filename.match(/\.(mp3|wav|ogg|m4a|aac)$/i)) && <div className="p-4">
                        ...
                        {/* Document Display */}
                        {(file.file_type === 'document' || file.mime_type?.includes('pdf') || file.mime_type?.includes('document') || file.mime_type?.includes('text') || file.mime_type?.includes('application/') || file.filename.match(/\.(pdf|doc|docx|txt)$/i)) && file.file_type !== 'image' && file.file_type !== 'video' && file.file_type !== 'audio' && <div className="aspect-video bg-muted flex flex-col items-center justify-center p-4">
