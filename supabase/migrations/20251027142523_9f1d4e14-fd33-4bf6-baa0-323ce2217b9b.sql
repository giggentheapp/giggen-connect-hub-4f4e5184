-- Fix get_public_profile to handle missing profile_settings gracefully
-- Default to showing profile if profile_settings doesn't exist (backwards compatibility)
CREATE OR REPLACE FUNCTION public.get_public_profile(target_user_id uuid)
RETURNS TABLE(id uuid, user_id uuid, display_name text, role text, avatar_url text, created_at timestamp with time zone, bio text, address text, latitude numeric, longitude numeric, is_address_public boolean)
LANGUAGE plpgsql
SECURITY DEFINER
