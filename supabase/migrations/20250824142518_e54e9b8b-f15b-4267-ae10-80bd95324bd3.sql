...
-- Step 2: Add profile_id column with foreign key reference to profiles
ALTER TABLE public.profile_tech_specs 
ADD COLUMN profile_id uuid REFERENCES public.profiles(user_id);

-- Step 3: Copy data from creator_id to profile_id
...
-- Step 5: Drop the old creator_id column (now that policies are gone)
ALTER TABLE public.profile_tech_specs 
