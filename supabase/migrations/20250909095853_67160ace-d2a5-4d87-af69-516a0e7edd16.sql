...
DROP POLICY IF EXISTS "Booking parties can view their bookings" ON public.bookings;
DROP POLICY IF EXISTS "Public can view published booking summaries" ON public.bookings;
DROP POLICY IF EXISTS "Only booking parties can view bookings" ON public.bookings;
DROP POLICY IF EXISTS "Only booking parties can update bookings" ON public.bookings;

-- Create comprehensive RLS policies for the new workflow
...
-- Policy for public viewing of published bookings (limited fields only through function)
CREATE POLICY "Public can view published booking info"
  ON public.bookings FOR SELECT
  USING (
...
DROP POLICY IF EXISTS "Booking parties can view change history" ON public.booking_changes;
...
CREATE POLICY "Booking parties can create changes"
...
CREATE POLICY "Booking parties can update changes"
