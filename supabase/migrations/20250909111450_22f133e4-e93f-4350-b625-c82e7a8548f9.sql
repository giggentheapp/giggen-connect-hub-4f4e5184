-- Create new enum with the updated lifecycle values
...
-- Migrate existing data to use the new enum values
UPDATE bookings SET 
  status = CASE 
    WHEN status::text = 'draft' THEN 'pending'::text
    WHEN status::text = 'pending' THEN 'pending'::text
    WHEN status::text = 'allowed' THEN 'allowed'::text
    WHEN status::text = 'approved' THEN 'both_parties_approved'::text
    WHEN status::text = 'published' THEN 'upcoming'::text
    WHEN status::text = 'confirmed' THEN 'completed'::text
    WHEN status::text = 'cancelled' THEN 'cancelled'::text
    WHEN status::text = 'rejected' THEN 'cancelled'::text
    WHEN status::text = 'deleted' THEN 'cancelled'::text
    ELSE 'pending'::text
  END::booking_status;
